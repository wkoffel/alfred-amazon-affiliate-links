...
if(!aff_code || aff_code.empty?)
  workflow << Alfred::Item.new(:valid => false, :title => "No Affiliate Code Set", :subtitle => "Run 'alink code your-code-tag' to setup", :icon_path => "icon.png")
elsif(!product_url || product_url.empty?)
  workflow << Alfred::Item.new(:valid => false, :title => "Generate Link", :subtitle => "Provide Product URL", :icon_path => "icon.png")
elsif(new_url = create_affiliate_link(product_url, aff_code))
  workflow << Alfred::Item.new(:arg => new_url, :title => "Generate Link", :subtitle => "with Amazon Affiliate Code", :icon_path => "icon.png")
else
  workflow << Alfred::Item.new(:valid => false, :title => "Invalid URL", :subtitle => "Provide Product URL", :icon_path => "icon.png")
end
